...
template class FootprintCollisionChecker<
    std::shared_ptr<costmap_2d::Costmap2D>>;
...
void GridCollisionChecker::setFootprint(const Footprint &footprint,
                                        const bool &radius,
                                        const double &possible_collision_cost) {
  std::cout << "setFootprint " << std::endl;
  ...
  for (auto p : footprint) {
    std::cout << "set fp = " << p.x << " " << p.y << std::endl;
  }
  ...
  for (unsigned int i = 0; i != angles_.size(); i++) {
    ...
    for (unsigned int j = 0; j < footprint_size; j++) {
      ...
      std::cout << "footprint point = " << new_pt.x << " " << new_pt.y
                << std::endl;
      oriented_footprint.push_back(new_pt);
    }
...
bool GridCollisionChecker::inCollision(const float &x, const float &y,
  ...
  // Check to make sure cell is inside the map            //需要优化。
  ...
  if (!footprint_is_radius_) {
    ...
    footprint_cost_ = static_cast<float>(
        costmap_->getCost(static_cast<unsigned int>(x + 0.5f),
    ...
    if (footprint_cost_ < possible_collision_cost_) {
      if (possible_collision_cost_ > 0.0f) {
        ...
        //   logger_, *clock_, 1000,
        //   "Inflation layer either not found or inflation is not set
