...
void Arc::setGoal(std::shared_ptr<Args> args) {
  args_ = std::dynamic_pointer_cast<ArcArgs>(args);
...
void Arc::update() {
  double v = args_->max_linear_velocity;
  double w =
      args_->max_linear_velocity / (rotationRadius() * sign(desired_distance_));
  ...
  if (distance > args_->max_angular_velocity) {
    ROS_WARN("Control(%s) jump distance(%f) from(%f) to(%f)", toDeg(distance),
             toDeg(last_pose_.z()), toDeg(current_pose.z()));
    distance = 0.0;
  }
...
double Arc::rotationRadius() const { return args_->rotation_radius; }
