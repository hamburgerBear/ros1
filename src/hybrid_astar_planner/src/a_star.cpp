  ...
  std::vector<std::tuple<float, float, float>> * expansions_log)
{
  ...
  // std::cout << "当前节点 = " << coords.x << " " << coords.y << std::endl;
  expansions_log->emplace_back(
    _costmap->getOriginX() + ((coords.x + 0.5) * _costmap->getResolution()),
    _costmap->getOriginY() + ((coords.y + 0.5) * _costmap->getResolution()),
    NodeT::motion_table.getAngleFromBin(coords.theta));
  // std::cout << "w当前节点 = " << _costmap->getOriginX() + ((coords.x + 0.5) * _costmap->getResolution())
  //           << " " << _costmap->getOriginY() + ((coords.y + 0.5) * _costmap->getResolution())
  //           << " " << NodeT::motion_table.getAngleFromBin(coords.theta) << std::endl;
}
  ...
  std::vector<std::tuple<float, float, float>> * expansions_log)
{
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    // std::cout << "当前节点id=" << current_node->getIndex() << std::endl;
...
{
  // std::cout << "添加节点 pose = " << node->pose.x << " " << node->pose.y << std::endl;
