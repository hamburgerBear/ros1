...
#include "hybrid_astar_planner/analytic_expansion.h"
      ...
      if (!analytic_nodes.empty()) {
        ...
        while (min_turn_rad < max_min_turn_rad) {
          ...
          if (node->motion_table.motion_model == MotionModel::DUBIN) {
            state_space = std::make_shared<DubinsStateSpace>(min_turn_rad);  
          } else {
            state_space = std::make_shared<ReedsSheppStateSpace>(min_turn_rad);  
          }
...
typename AnalyticExpansion<NodeT>::AnalyticExpansionNodes AnalyticExpansion<NodeT>::getAnalyticPath(
  ...
  const std::shared_ptr<StateSpace> & state_space)
{

  double from[3] = { node->pose.x, node->pose.y, node->motion_table.getAngleFromBin(node->pose.theta) };
  double to[3] = { goal->pose.x, goal->pose.y, node->motion_table.getAngleFromBin(goal->pose.theta) };
  double s[3];
  ...
  state_space->init(from, to);
  float d = state_space->distance();
  ...
  for (float i = 1; i <= num_intervals; i++) {
    ...
    theta = (s[2] < 0.0) ? (s[2] + 2.0 * M_PI) : s[2];
    ...
    if (node_getter(index, next)) {
      ...
      proposed_coordinates = {static_cast<float>(s[0]), static_cast<float>(s[1]), angle};
