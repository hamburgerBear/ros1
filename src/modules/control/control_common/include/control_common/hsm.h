...
struct Transition {
  ...
  Transition() : mTransitionType(Transition::No), mStateFactory(0) {}
  ...
  Transition(Transition::Type transitionType, const StateFactory& stateFactory,
             OnEnterArgsFunc onEnterArgsFunc)
 ...
 private:
  ...
  OnEnterArgsFunc
      mOnEnterArgsFunc;  // Optional: set if transition specifies arguments
};
...
Transition SiblingTransition(Args&&... args) {
  return Transition(Transition::Sibling, GetStateFactory<TargetState>(),
                    detail::GenerateOnEnterArgsFunc<TargetState>(
                        std::forward<Args>(args)...));
}

// Deprecated after v1.5 upon realizing that it's not possible to bind to the
// correct OnEnter for state overrides since the actual target state is not
// known at compile time, but rather at runtime.
...
Transition SiblingTransition(const StateOverride<TargetState>& stateOverride,
                             T1&& arg1, Args&&... args);
...
template <typename TargetState, typename T1, typename... Args>
HSM_DEPRECATED(
    "Passing state args to state overrides is not supported")  // See details on
                                                               // SiblingTransition
                                                               // version
Transition InnerTransition(const StateOverride<TargetState>& stateOverride,
                           T1&& arg1, Args&&... args);
...
HSM_DEPRECATED(
    "Passing state args to state overrides is not supported")  // See details on
                                                               // SiblingTransition
                                                               // version
Transition InnerEntryTransition(const StateOverride<TargetState>& stateOverride,
                                T1&& arg1, Args&&... args);
...
struct State {
  ...
  template <typename StateType>
  hsm_bool IsInOuterState() const {
    return GetOuterState<StateType>() != 0;
  }

  template <typename StateType>
  hsm_bool IsInInnerState() const {
    return GetInnerState<StateType>() != 0;
  }

  template <typename StateType>
  hsm_bool IsInImmediateInnerState() const {
...
class StateMachine {
 public:
  ...
  template <typename InitialStateType>
  HSM_DEPRECATED(
      "Initialize should no longer accept debug info. Use SetDebugInfo "
      "instead.")
...
inline State* StateMachine::GetState(StateTypeId stateType) {
  for (size_t i = 0; i < mStateStack.size(); ++i) {
    ...
    if (state->GetStateType() == stateType) return state;
  }
  return 0;
}
...
inline void StateMachine::CreateAndPushInitialState(
      ...
      case Transition::Inner: {
        if (State* innerState = GetStateAtDepth(depth + 1)) {
          if (transition.GetTargetStateType() == innerState->GetStateType()) {
            ...
            State* targetState =
                detail::CreateState(transition, this, depth + 1);
